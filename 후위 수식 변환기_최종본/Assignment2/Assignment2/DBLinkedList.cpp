...
//Node 객체의 메서드
void Node<Data>::set_data(Data new_data)//노드 데이터 입력
{
	data = new_data;
}

void Node<Data>::set_Rlink(Node* new_link)//오른쪽 노드 연결
{
	next = new_link;
}

void Node<Data>::set_Llink(Node* new_link)//왼쪽 노드 연결
{
	prev = new_link;
}

Data Node<Data>::data_show() //노드 데이터값 반환
{
	return data;
}

Node<Data>* Node<Data>::Rlink_show()//오른쪽 노드 주소값 반환
{
	return next;
}

Node<Data>* Node<Data>::Llink_show()//왼쪽 노드 주소값 반환
{
	return prev;
}

//DBLinkList객체의 메서드
void OurSet::set_link(Node<Data>* new_link)// 현재 가리키는 노드 세팅
{
	cur = new_link;
}
...
void list_insert(List* plist,Node<Data>*previous_ptr, Data entry)
{
	if (list_search(plist, entry)==NULL)
	{
		if (plist->head != NULL)
		{
			...
			Node<Data>* insert_ptr;
			insert_ptr = new Node<Data>;
 ...
 Node<Data>* list_search(List* plist, const Data& target) //특정값을 찾는다.
{
	Node<Data>* cursor;
 ...
 Node<Data>* list_locate(List*plist, Node<Data>* head_ptr, Data position) //특정위치를 찾는다.
 {
	 Node<Data>* cursor = head_ptr;
	 Data i;
 ...
 void list_head_remove(List* plist, Node<Data>* head_ptr) //헤드부분 노드를 삭제
 {
	 Node<Data>* remove_ptr;
...
void list_remove(List* plist, Node<Data>* previous_ptr) //previous_ptr바로 다음 노드를 삭제
{
	Node<Data>* remove_ptr;
